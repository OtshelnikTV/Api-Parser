...
export class FileService {
    ...
    async indexProject(projectName, projectState) {
        ...
        if (openapiContent === null) {
            openapiContent = await this.getFileContent(projectState.projectRoot + '/openapi.yaml')
                || await this.getFileContent(projectState.projectRoot + '/openapi.yml');
            if (!openapiContent) throw new Error('OpenAPI файл не найден');
        }
        
        // Парсим пути из openapi.yaml
        const pathEntries = this.parsePathsFromOpenapi(openapiContent);
        
        // ❌ УБИРАЕМ Promise.all — НЕ загружаем файлы!
        // ✅ Только сохраняем метаданные
        for (const [apiPath, refPath] of Object.entries(pathEntries)) {
            ...
            projectState.pathsFolders[endpointName] = {
                apiPath: apiPath,
                refPath: refPath,        // ← сохраняем путь
                methods: null,           // ← не знаем методов, пока не загрузили
                flat: true,
                flatContent: null,       // ← содержимое загрузится позже
    ...
    async loadEndpointContent(endpointName, projectState) {
        const endpoint = projectState.pathsFolders[endpointName];
        if (!endpoint) throw new Error('Endpoint не найден');
        
        // Если уже загружен — не загружаем повторно
        if (endpoint.flatContent) return endpoint;
        
        const fullPath = projectState.projectRoot + '/' + endpoint.refPath;
        ...
        endpoint.methods = foundMethods;
        endpoint.flatContent = content;
